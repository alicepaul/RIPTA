...
is_transfer <- function(idx, data, max_time=7320) {
  #' Determines if a ride in the Wave data is a transfer.
  #'
  #' @param idx integer - Index of the ride in the data
  #' @param data data frame - Wave data grouped by
  #' Card.Number and sorted by time
  #' @param max_time integer - The maximum time (in
  #' seconds) allowed between two rides to be considered
  #' a transfer; the default is 7320 (122 minutes)
  #'
  #' @returns numeric vector - The first value indicates
  #' if the ride is a transfer and the second value indicates
  #' if the ride is a transfer AND its route number differs
  #' from that of the non-transfer ride.
  
  # First row cannot be a transfer because the data is
  # grouped by Card.Number and sorted by time
  if (idx == 1) {
    return(c(0, 0))
  }
  ...
  while (idx > 1) {
    idx <- idx - 1
    prev_ride <- data[idx,]
    # Transfer must be for the same rider
    if (curr_ride$Card.Number != prev_ride$Card.Number) {
      return(c(0, 0))
    }
    # Transfer must be within max time
    if (as.numeric(curr_ride$Time) - as.numeric(prev_ride$Time) > max_time) {
      return(c(0, 0))
    }
    # At this point, the previous ride shares the
    # same card number and is within max time
    
    # Previous ride was not a transfer, therefore
    # this ride is
    if (prev_ride$Transfer == 0) {
      if (prev_ride$Route.Number != curr_ride$Route.Number) {
        return(c(1, 1))
      }
      return(c(1, 0))
    }
    # Previous ride was a transfer, must
    # check older rides for non-transfer
  }
  return(c(0, 0))
}
